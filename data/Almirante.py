...
class Almirante(Inimigos):
    def __init__(self, x, y, limiteAltura, limiteLargura, tirosGroup, tela):
        super().__init__(x, y, limiteAltura, limiteLargura, tirosGroup, "./assets/boss_barco_marinha.png", 480)
        self.combustivel_maximo = self.combustivel_base = 100
        self.combustivel_restante = 100
        self.tela = tela

    def healtBar(self, tela):
        pygame.draw.rect(tela, (255,0,0), (self.rect.x, (self.rect.bottom-50), self.rect.width, 15))
        if self.combustivel_restante > 0:
            pygame.draw.rect(tela, (0,255,0), (self.rect.x, (self.rect.bottom-50), int(self.rect.width * (self.combustivel_restante / self.combustivel_base)), 15))
